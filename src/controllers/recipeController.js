...
export const createRecipe = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      const mediaSql =
        "INSERT INTO midia (id_usuario_upload, url_arquivo, tipo_arquivo) VALUES (?, ?, ?)";
      const tipo_arquivo = req.file.mimetype.startsWith("image")
        ? "imagem"
        : "video";
      const relativePath = path
        .relative(process.cwd(), req.file.path)
        .replace(/\\/g, "/");
      const [mediaResult] = await connection.query(mediaSql, [
        id_usuario_criador,
        relativePath,
        tipo_arquivo,
      ]);
    ...
    for (const grupo of grupos_ingredientes || []) {
      const groupSql =
        "INSERT INTO grupos_ingredientes (id_receita, titulo, ordem) VALUES (?, ?, ?)";
      const [groupResult] = await connection.query(groupSql, [
        id_receita,
        grupo.titulo,
        grupo.ordem,
      ]);
      ...
      for (const ingrediente of grupo.ingredientes || []) {
        const ingSql =
          "INSERT INTO ingredientes (id_grupo, descricao, observacao, ordem) VALUES (?, ?, ?, ?)";
        await connection.query(ingSql, [
          id_grupo,
    ...
    for (const passo of passos_preparo || []) {
      const passoSql =
        "INSERT INTO passos_preparo (id_receita, descricao, observacao, ordem) VALUES (?, ?, ?, ?)";
      await connection.query(passoSql, [
        id_receita,
        passo.descricao,
        passo.observacao,
        passo.ordem,
    ...
    for (const id_tag of tags || []) {
      const tagSql =
        "INSERT INTO receita_tags (id_receita, id_tag) VALUES (?, ?)";
...
export const getRecipeById = async (req, res) => {
  ...
  try {
    ...
    const [grupos] = await connection.query(
      "SELECT * FROM grupos_ingredientes WHERE id_receita = ? ORDER BY ordem",
      [id]
    );
    for (const grupo of grupos) {
      const [ingredientes] = await connection.query(
        "SELECT * FROM ingredientes WHERE id_grupo = ? ORDER BY ordem",
        [grupo.id]
      );
      grupo.ingredientes = ingredientes;
    }

    const [passos] = await connection.query(
      "SELECT * FROM passos_preparo WHERE id_receita = ? ORDER BY ordem",
      [id]
    );
    const [tags] = await db.query(
      "SELECT t.id, t.nome FROM receita_tags rt JOIN tags t ON rt.id_tag = t.id WHERE rt.id_receita = ?",
      [id]
    );
    ...
    const categoria = receita.categoria_id
      ? {
          ...
          imagem_url: receita.categoria_imagem_url
            ? String(receita.categoria_imagem_url).replace(/\\/g, "/")
            : null,
        }
      : null;
    ...
    const fullRecipe = {
      ...
      imagem_url: receita.imagem_url
        ? String(receita.imagem_url).replace(/\\/g, "/")
        : null,
      criador: criador,
      categoria: categoria,
  ...
  try {
    ...
    if (req.file) {
      ...
      if (existingRecipe.length > 0 && existingRecipe[0].id_midia_principal) {
        ...
        const mediaUpdateSql =
          "UPDATE midia SET id_usuario_upload = ?, url_arquivo = ?, tipo_arquivo = ? WHERE id = ?";
        await connection.query(mediaUpdateSql, [
          id_usuario_criador,
          relativePath,
          tipo_arquivo,
          existingRecipe[0].id_midia_principal,
        ...
        const mediaInsertSql =
          "INSERT INTO midia (id_usuario_upload, url_arquivo, tipo_arquivo) VALUES (?, ?, ?)";
        const [mediaInsertResult] = await connection.query(mediaInsertSql, [
          id_usuario_criador,
          relativePath,
          tipo_arquivo,
        ]);
        ...
        const updateRecipeMediaSql =
          "UPDATE receitas SET id_midia_principal = ? WHERE id = ? AND id_usuario_criador = ?";
        await connection.query(updateRecipeMediaSql, [
          new_id_midia_principal,
          id,
          id_usuario_criador,
        ]);
      }
    ...
    if (typeof id_produtor !== "undefined") {
      fieldsToUpdate.push("id_produtor = ?");
      values.push(id_produtor);
    }
    if (typeof dificuldade !== "undefined") {
      fieldsToUpdate.push("dificuldade = ?");
      values.push(dificuldade);
    }
    if (typeof tempo_preparo_min !== "undefined") {
      fieldsToUpdate.push("tempo_preparo_min = ?");
      values.push(tempo_preparo_min);
    }
    if (typeof tempo_cozimento_min !== "undefined") {
      fieldsToUpdate.push("tempo_cozimento_min = ?");
      values.push(tempo_cozimento_min);
    }
    if (typeof porcoes !== "undefined") {
      fieldsToUpdate.push("porcoes = ?");
      values.push(porcoes);
    }
    ...
    if (typeof calorias_kcal !== "undefined") {
      ...
      values.push(calorias_kcal);
    }
    if (typeof proteinas_g !== "undefined") {
      fieldsToUpdate.push("proteinas_g = ?");
      values.push(proteinas_g);
    }
    if (typeof carboidratos_g !== "undefined") {
      fieldsToUpdate.push("carboidratos_g = ?");
      values.push(carboidratos_g);
    }
    if (typeof gorduras_g !== "undefined") {
      fieldsToUpdate.push("gorduras_g = ?");
      values.push(gorduras_g);
    }
    ...
    if (fieldsToUpdate.length > 0) {
      ...
      if (recipeUpdateResult.affectedRows === 0) {
        ...
        return res.status(404).json({
          message:
            "Receita não encontrada ou você não tem permissão para editá-la.",
        });
      }
    }
    ...
    if (passos_preparo !== undefined) {
      await connection.query(
        "DELETE FROM passos_preparo WHERE id_receita = ?",
        [id]
      );
      for (const passo of passos_preparo || []) {
        const passoSql =
    ...
    if (tags !== undefined) {
      await connection.query("DELETE FROM receita_tags WHERE id_receita = ?", [
        id,
      ]);
      for (const id_tag of tags || []) {
        const tagSql =
  ...
  try {
    ...
    if (tagsQuery) {
      const tagIds = tagsQuery
        ...
        .filter((id) => !isNaN(id));
      if (tagIds.length > 0) {
        whereClause += ` AND r.id IN (SELECT id_receita FROM receita_tags WHERE id_tag IN (?) GROUP BY id_receita HAVING COUNT(DISTINCT id_tag) = ?)`;
...
export const deactivateRecipe = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      'UPDATE receitas SET status = "inativo" WHERE id = ?',
      [id]
    );
...
export const activateRecipe = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const [result] = await connection.query(
      'UPDATE receitas SET status = "ativo" WHERE id = ?',
      [id]
    );
...
export const deleteRecipe = async (req, res) => {
  ...
  try {
    ...
    await connection.query("DELETE FROM passos_preparo WHERE id_receita = ?", [
      id,
    ]);
    await connection.query("DELETE FROM receita_tags WHERE id_receita = ?", [
      id,
    ...
    const [result] = await connection.query(
      "DELETE FROM receitas WHERE id = ? AND id_usuario_criador = ?",
    ...
    if (result.affectedRows === 0) {
      ...
      return res.status(404).json({
        message:
          "Receita não encontrada ou você não tem permissão para deletá-la.",
      });
    }
...
export const deleteRecipeImage = async (req, res) => {
  ...
  try {
    ...
    const [recipe] = await connection.query(
      "SELECT id_midia_principal FROM receitas WHERE id = ? AND id_usuario_criador = ?",
      [id, id_usuario_criador]
    );
    ...
    if (recipe.length === 0) {
      ...
      return res.status(404).json({
        message:
          "Receita não encontrada ou você não tem permissão para esta ação.",
      });
    }
    ...
    if (id_midia_principal) {
      ...
      await connection.query(
        "UPDATE receitas SET id_midia_principal = NULL WHERE id = ?",
        [id]
      );
      await connection.query("DELETE FROM midia WHERE id = ?", [
        id_midia_principal,
    ...
    res
      .status(200)
      .json({ message: "Imagem da receita deletada com sucesso." });
  } catch (error) {
    await connection.rollback();
...
export const getUsedCategories = async (req, res) => {
  try {
    const [categories] = await db.query(`
            SELECT DISTINCT
                c.id, c.nome
            FROM categorias_receitas AS c
            JOIN receitas AS r ON c.id = r.id_categoria
  ...
  try {
    const [tags] = await db.query(`
            SELECT DISTINCT
                t.id, t.nome
            FROM tags AS t
            JOIN receita_tags AS rt ON t.id = rt.id_tag
            JOIN receitas AS r ON rt.id_receita = r.id
            WHERE r.status = 'ativo'
            ORDER BY t.nome;
        `);
    res.json(tags);
